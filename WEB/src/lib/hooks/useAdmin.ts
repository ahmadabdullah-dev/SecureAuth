...
export const useAdmin = (pagination?: PaginationParams) => {
    ...
    queryFn: async () =>
      await agent
        .get<
          Result<PaginatedList<UserDto>>
        >("/admin/users", { params: pagination })
        .then((res) => res.data),
    enabled: !!pagination,
    staleTime: 5 * 60 * 1000,
  });
  const DeleteUserAsync = useMutation({
    mutationFn: async (creds: DeleteUserDto) => {
      const response = await agent.delete("/admin/delete-user", {data: creds});
      return response.data;
    },
  });
  const AssignToAdminRoleAsync = useMutation({
    mutationFn: async (creds: AssignToAdminRoleDto) => {
      const response = await agent.patch("/admin/assign-to-admin-role", creds);
      return response.data;
    },
  });
  const AssignToMemberRoleAsync = useMutation({
    mutationFn: async (creds: AssignToMemberRoleeDto) => {
      const response = await agent.patch("/admin/assign-to-member-role", creds);
      return response.data;
    },
  });
  return {
