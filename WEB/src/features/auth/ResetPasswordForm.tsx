...
export default function ResetPasswordForm() {
  ...
  const onSubmit = (creds: ResetPasswordDto) => {
    resetPasswordAsync.mutate(creds, {
        onSuccess: () => {
        reset();
      },
    });
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)}>
            <Stack spacing={2}>
              ...
              <Button
                ...
                variant="contained"
                fullWidth
                disabled={resetPasswordAsync.isPending}
              >
                {resetPasswordAsync.isPending ? (
                  <CircularProgress size={24} color="inherit" />
                ) : (
                  "Continue"
                )}
              </Button>
              {resetPasswordAsync.data?.isSuccess && (
                <Alert severity="success">
                  {resetPasswordAsync.data.value}
                </Alert>
              )}
              {resetPasswordAsync.error && (
                <Alert severity="error">
                  {resetPasswordAsync.error.message}
                </Alert>
              )}
              <Link
                component="button"
